...
for search_link in links_list:
    ...
    for article in articles:
        try:
            link = article.find_element_by_css_selector(
                "div.cl-paper-row-serp-papers__paper-row-paper-row-normal > a[data-selenium-selector=title-link]").get_attribute(
                "href")
            articles_links.append(url + link)
            print("[DEBUG]: Article url: " + link)
        except:
            pass
    ...
    for link in articles_links:
        ...
        print("article name = " + title)

        authors = []
        print("DEBUG: find authors...")
        for author in driver.find_elements_by_css_selector("span[data-selenium-selector=author-list]"):
            authors.append(author.text)
            print("[DEBUG]: author text = " + author.text)

        paper_link = driver.find_element_by_css_selector("a[data-selenium-selector=paper-link][data-heap-link-type=publisher]").get_attribute("href")
        description = driver.find_element_by_css_selector("span[data-selenium-selector=text-truncator-text]")
        citations = driver.find_element_by_css_selector("div[data-selenium-selector=[scorecard-body] > span.scorecard-stat__headline").text
        ...
        try:
            ...
            driver.find_element_by_css_selector(
                "a[data-selenium-selector=paper-link][data-heap-id=paper_link_target]").click()
            time.sleep(5)
